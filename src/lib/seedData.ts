...
export async function seedInitialFirestoreData() {
  try {
    // Check if initial seeding has already been completed once
    ...
    try {
      ...
      // If offline or network error on first read, proceed cautiously or skip
      console.warn("Unable to check seed status (offline/network issue):", e);
    }
    ...
    try {
      ...
      console.warn("Seeding petugas skipped (offline/network):", pErr);
    }
    ...
    try {
      ...
      console.warn("Seeding rencana_bulanan skipped:", rbErr);
    }
    ...
    try {
      ...
      console.warn("Seeding rencana_harian skipped:", rhErr);
    }
    ...
    try {
      ...
      console.warn("Seeding laporan skipped:", lapErr);
    }
    ...
    try {
      ...
      console.warn("Seeding kegiatan_harian skipped:", khErr);
    }
    ...
    try {
      ...
      console.warn("Seeding modul_p2k2 skipped:", p2k2Err);
    }

    // Flag database as seeded
    try {
      await setDoc(seedRef, { isSeeded: true, seededAt: new Date().toISOString() }, { merge: true });
    } catch (sErr) {
      console.warn("Setting seed status flag skipped:", sErr);
    }
  } catch (err) {
